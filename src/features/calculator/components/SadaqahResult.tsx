  ...
  return (
    <div className="flex flex-col gap-3">
      <div className="flex flex-col items-center gap-1.5 rounded-2xl border border-indigo-200 bg-white px-4 py-6 text-center dark:border-indigo-800 dark:bg-slate-800">
        <p className="text-xs font-medium uppercase tracking-[0.15em] text-slate-500">{t('sadaqahResultTitle')}</p>
        <p className="text-3xl font-medium text-indigo-600 dark:text-indigo-400">{guidance?.displayName ?? hijri.monthName.en}</p>
      ...
      {text ? (
        ...
        <p className="text-base text-slate-500">{t('sadaqahUnavailable')}</p>
      )}

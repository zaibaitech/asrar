    ...
    <div className="fixed inset-0 z-50 flex items-end justify-center bg-black/60 sm:items-center" onClick={onClose}>
      ...
      >
        <div className="flex items-center justify-between border-b border-slate-200 px-4 py-3 dark:border-slate-700">
          <h2 className="text-base font-medium text-slate-900 dark:text-slate-100">{t('chooseFromList')}</h2>
        ...
        <div className="flex-1 overflow-y-auto p-2">
          {results.length === 0 ? (
            ...
            results.map((item) => (
              ...
              >
                <span className="text-base text-slate-600 dark:text-slate-300">{item.latin}</span>
                <span dir="rtl" className="text-lg text-slate-900 dark:text-slate-100">
                  {item.arabic}
                </span>
              </button>
            ))

  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-1.5 rounded-2xl border border-gold/20 bg-navy-card px-4 py-6 text-center">
        ...
        <p dir="rtl" lang="ar" className="text-4xl font-medium text-gold font-arabic">
          ...
          <p dir="ltr" className="flex flex-wrap items-center justify-center gap-x-1.5 gap-y-1 text-sm text-slate-400">
            {letterBreakdown.map((lv, i) => (
              <span key={i} className="flex items-center gap-1.5">
                ...
                <span dir="rtl" className="text-slate-200 font-arabic">
                  {lv.char}
                </span>
                <span>= {lv.value}</span>
              </span>
      ...
      {profile.letterFrequency.length > 0 && (
        <Card className="flex flex-col gap-3">
          ...
          <p dir="ltr" className="flex flex-wrap justify-center gap-2 text-2xl text-slate-100 font-arabic">
            {letterBreakdown.map((lv, i) => (
              <span key={i} dir="rtl">
                {lv.char}
              </span>
            ...
            {profile.letterFrequency.map((f) => {
              ...
              return (
                <div key={f.letter} className="flex items-center gap-2">
                  <span dir="rtl" className="w-5 shrink-0 text-center text-base text-slate-100 font-arabic">
                    {f.letter}
                  </span>
                  <span className="w-8 shrink-0 text-right text-xs text-slate-500">{f.value}</span>
                  <div className="h-2 flex-1 overflow-hidden rounded-full bg-white/5">
                    <div className="h-full rounded-full bg-gold" style={{ width: `${(f.count / max) * 100}%` }} />
                  </div>
          ...
          <div className="flex flex-col divide-y divide-white/5">
            {profile.letterFrequency.map((f) => (
              <div key={f.letter} className="flex items-center justify-between py-2 first:pt-0 last:pb-0">
                <span dir="rtl" className="text-xl text-gold font-arabic">
                  {f.letter}
                </span>
      ...
      {nameInsights && (
        <>
          ...
          <Card className="flex flex-col gap-2">
            ...
            <p className="text-lg font-semibold text-gold">{pick(uiLocale, nameInsights.archetype.title)}</p>
            <p className="text-sm text-slate-300">{pick(uiLocale, nameInsights.archetype.description)}</p>
            <div>
            ...
            <div className="border-t border-white/5 pt-2">
              ...
              <p className="text-sm text-slate-300">{pick(uiLocale, nameInsights.guidance)}</p>
            </div>
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div>
                <p className="font-medium text-slate-500">{t('bestTimeWindow')}</p>
                <p className="text-slate-300">{pick(uiLocale, nameInsights.bestTime)}</p>
              </div>
              <div>
                <p className="font-medium text-slate-500">{t('powerDays')}</p>
                <p className="text-slate-300">{pick(uiLocale, nameInsights.powerDay)}</p>
              </div>
            </div>
          </Card>
            ...
            <Card className="flex flex-col gap-3">
              ...
              <div className="flex flex-col gap-2">
                {nameInsights.divineNameMatches.map((m, i) => {
                  ...
                  return (
                    <div key={m.number} className={`rounded-xl border px-3 py-2.5 ${isClosest ? 'border-gold/40 bg-gold/5' : 'border-white/10 bg-navy/60'}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p dir="rtl" className="text-xl text-gold font-arabic">
                            {m.arabic}
                          </p>
        ...
        <Card className="flex flex-col gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap gap-1.5">
                {phraseInsights.repeatedLetters.map((f) => (
                  <span key={f.letter} className="flex items-center gap-1.5 rounded-lg border border-white/10 bg-navy px-2 py-1 text-xs">
                    <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: ELEMENT_BAR_COLOR[f.element] }} />
                    <span dir="rtl" className="text-sm text-slate-100 font-arabic">
                      {f.letter}
                    </span>
          ...
          {phraseInsights.sacredMatch && (
            <div>
              ...
              <p className="text-sm text-slate-300">{pick(uiLocale, phraseInsights.sacredMatch.significance)}</p>
            </div>
          )}
        </Card>
      )}
      ...
      {dhikrCounts && (
        <Card className="flex flex-col gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-slate-300">{DHIKR_TIMING.map((t2) => pick(uiLocale, t2)).join(' · ')}</p>
          </div>
          <div>

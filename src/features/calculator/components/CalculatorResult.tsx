...
function Pill({ children }: { children: React.ReactNode }) {
  return <span className="rounded-full border border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-900/30 px-2.5 py-1 text-xs text-indigo-600 dark:text-indigo-400">{children}</span>;
}
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex flex-col items-center gap-1.5 rounded-2xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-slate-800 px-4 py-6 text-center">
        ...
        <p dir="rtl" lang="ar" className="text-4xl font-medium text-indigo-600 dark:text-indigo-400 font-arabic">
          {profile.normalizedText}
        </p>
        {latin && <p className="text-sm text-slate-600 dark:text-slate-300">{latin}</p>}
      ...
      <div className="flex flex-col items-center gap-2 rounded-2xl border border-indigo-200 dark:border-indigo-800 bg-white dark:bg-slate-800 px-4 py-5 text-center">
        <p className="text-xs font-semibold uppercase tracking-[0.15em] text-indigo-600 dark:text-indigo-400">{t('numericalSignature')}</p>
        <p className="text-xs text-slate-500">{t('abjadKabirLabel')}</p>
        <p className="text-5xl font-bold text-indigo-600 dark:text-indigo-400">{profile.kabir}</p>
        {letterBreakdown.length > 0 && (
          <p dir="ltr" className="flex flex-wrap items-center justify-center gap-x-1.5 gap-y-1 text-sm text-slate-500 dark:text-slate-400">
      ...
      {profile.letterFrequency.length > 0 && (
        <Card className="flex flex-col gap-3">
          ...
          <p dir="ltr" className="flex flex-wrap justify-center gap-2 text-2xl text-slate-900 dark:text-slate-100 font-arabic">
            {letterBreakdown.map((lv, i) => (
              <span key={i} dir="rtl">
                {lv.char}
              </span>
            ...
            {profile.letterFrequency.map((f) => {
              ...
              return (
                <div key={f.letter} className="flex items-center gap-2">
                  <span dir="rtl" className="w-5 shrink-0 text-center text-base text-slate-900 dark:text-slate-100 font-arabic">
                    {f.letter}
                  </span>
                  ...
                  <span className="w-8 shrink-0 text-xs text-slate-500 dark:text-slate-400">×{f.count}</span>
                </div>
              );
            })}
          </div>
          <div className="flex items-center justify-between border-t border-slate-100 dark:border-white/5 pt-2 text-xs text-slate-500">
            <span>
        ...
        <div className="grid grid-cols-3 gap-2">
          {numbers.map((n) => (
            ...
            >
              ...
              <p className="text-lg font-semibold text-indigo-600 dark:text-indigo-400">{n.value}</p>
              <p className="text-[10px] uppercase tracking-wide text-slate-500">{n.label}</p>
            </button>
          ))}
        </div>
        {openMetricData && (
          <div className="rounded-lg bg-slate-100 dark:bg-slate-900/60 p-2.5 text-xs leading-relaxed text-slate-500 dark:text-slate-400">
        ...
        <div className="flex flex-col gap-2">
          {elements.map((el) => (
            <div key={el} className="flex items-center gap-2">
              ...
              <span className="w-9 shrink-0 text-right text-xs text-slate-500 dark:text-slate-400">{profile.elemental.percents[el]}%</span>
            </div>
          ))}
        </div>
          ...
          <div className="flex flex-col divide-y divide-slate-100 dark:divide-white/5">
            {profile.letterFrequency.map((f) => (
              <div key={f.letter} className="flex items-center justify-between py-2 first:pt-0 last:pb-0">
                <span dir="rtl" className="text-xl text-indigo-600 dark:text-indigo-400 font-arabic">
                  {f.letter}
                </span>
      ...
      <Card className="flex flex-col items-center gap-2 text-center">
        ...
        {profile.zodiac ? (
          <>
            <span className="text-3xl text-indigo-600 dark:text-indigo-400">{profile.zodiac.symbol}</span>
            <p className="text-lg font-semibold text-indigo-600 dark:text-indigo-400">{profile.zodiac[locale]}</p>
      ...
      {nameInsights && (
        <>
          ...
          <Card className="flex flex-col gap-2">
            ...
            <p className="text-lg font-semibold text-indigo-600 dark:text-indigo-400">{pick(uiLocale, nameInsights.archetype.title)}</p>
            <p className="text-sm text-slate-600 dark:text-slate-300">{pick(uiLocale, nameInsights.archetype.description)}</p>
            <div>
            ...
            <div className="border-t border-slate-100 dark:border-white/5 pt-2">
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">{pick(uiLocale, nameInsights.guidance)}</p>
            </div>
                  ...
                  return (
                    <div key={m.number} className={`rounded-xl border px-3 py-2.5 ${isClosest ? 'border-indigo-300 dark:border-indigo-700 bg-indigo-50 dark:bg-indigo-900/20' : 'border-slate-200 dark:border-slate-700 bg-slate-100 dark:bg-slate-900/60'}`}>
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p dir="rtl" className="text-xl text-indigo-600 dark:text-indigo-400 font-arabic">
                            {m.arabic}
                          </p>
                          <p className="text-sm font-medium text-slate-700 dark:text-slate-200">{m.transliteration}</p>
                          {meaning && <p className="text-xs text-slate-500">{meaning}</p>}
                        </div>
                        <div className="flex shrink-0 flex-col items-end gap-1">
                          <span className="text-sm font-semibold text-indigo-600 dark:text-indigo-400">{m.abjadValue}</span>
                          <span
                      ...
                      <div className="mt-2 flex flex-wrap gap-x-3 gap-y-0.5 border-t border-slate-100 dark:border-white/5 pt-1.5 text-[11px] text-slate-500">
                        <span>
        ...
        <Card className="flex flex-col gap-3">
            ...
            <div>
              ...
              <div className="mt-1 flex flex-wrap gap-1.5">
                {phraseInsights.repeatedLetters.map((f) => (
                  <span key={f.letter} className="flex items-center gap-1.5 rounded-lg border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 px-2 py-1 text-xs">
                    <span className="h-1.5 w-1.5 rounded-full" style={{ backgroundColor: ELEMENT_BAR_COLOR[f.element] }} />
                    <span dir="rtl" className="text-sm text-slate-900 dark:text-slate-100 font-arabic">
                      {f.letter}
                    </span>
          ...
          {phraseInsights.sacredMatch && (
            <div>
              ...
              <p className="text-sm text-slate-600 dark:text-slate-300">{pick(uiLocale, phraseInsights.sacredMatch.significance)}</p>
            </div>
          )}
        </Card>
      )}
      ...
      {dhikrCounts && (
        <Card className="flex flex-col gap-3">
          ...
          <InfoDisclosure label={t('dhikrWhyThese')}>{t('dhikrWhyTheseBody')}</InfoDisclosure>
          <div>
            <p className="mb-1 text-xs font-medium text-slate-500 dark:text-slate-400">{t('bestTimesToPractice')}</p>
            <p className="text-sm text-slate-600 dark:text-slate-300">{DHIKR_TIMING.map((t2) => pick(uiLocale, t2)).join(' · ')}</p>
          </div>
          <div>
            <p className="mb-1 text-xs font-medium text-slate-500 dark:text-slate-400">{t('practiceGuidance')}</p>
            <p className="text-xs text-slate-500">
      ...
      <Card className="flex flex-col gap-3">
        <div>
          ...
          <ul className="mt-1.5 flex flex-col gap-1">
            {tellsItems.map((item) => (
              <li key={item} className="flex items-start gap-1.5 text-xs text-slate-600 dark:text-slate-300">

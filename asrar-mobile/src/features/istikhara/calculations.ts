...
import { calculateAbjadValue, modIndex, validateName } from '../../utils/abjad';
...
export function calculateIstikhara(
  ...
  if (!validateName(personName)) {
    throw new Error('Person name contains invalid characters');
  }
  if (!validateName(motherName)) {
    throw new Error('Mother name contains invalid characters');
  }
...
export { validateName } from '../../utils/abjad';
